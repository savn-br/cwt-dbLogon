<template lang="pug">
#approvalDelegationModal.approval-delegation-modal-wrapper
  .card
  .card-header
  .card-content
    form.tw-grid(name='perfil')
      b-field.mx-2(label='Register')
        b-input(v-model='perfil.register', size='is-small')
      b-field.mx-2(label='E-mail')
        b-input(v-model='perfil.email', size='is-small')
      b-field.mx-2(label='Name')
        b-input(v-model='perfil.name', size='is-small')
      b-field.mx-2(label='Data inicial')
        b-input(v-model='perfil.initial_date', type='date', size='is-small')
      b-field.mx-2(label='Data final')
        b-input(v-model='perfil.end_date', type='date', size='is-small')
  .card-footer.tw-px-6.tw-py-4.tw-flex.tw-justify-end
    .wrapper-buttons
      b-button.tw-mr-4(type='is-danger', @click='$emit("close")') Cancelar
      b-button(type='is-primary') Salvar
</template>

<script>
export default {
  name: 'ApprovalDelegationModal',
  components: {},
  props: {},
  data() {
    return {
      perfil: {
        register: '',
        email: '',
        name: '',
        initial_date: new Date(),
        end_date: new Date(),
      },
    }
  },
  computed: {},
  watch: {},
  mounted() {},
  created() {},
  methods: {},
}
</script>

<style lang="scss" scoped>
.approval-delegation-modal-wrapper {
  max-width: px2rem(875);
  background: white;
  margin: 0 auto;
  form {
    grid-template-columns: repeat(auto-fill, minmax(px2rem(280), 1fr));
  }
}
</style>

<style lang="scss">
.approval-delegation-modal-wrapper {
  label {
    font-size: 0.75rem;
  }
}
</style>
